...
# --- 1. Konfiguráció és API kulcs kezelése (Streaminghez szükséges beállítás) ---

# API kulcs betöltése: Streamlit Secrets > környezeti változó
try:
    if "GEMINI_API_KEY" in st.secrets:
        ...
        st.error("Hiányzik a GEMINI_API_KEY. Kérlek, konfiguráld a Streamlit Secrets-ben!")
        st.stop()
except Exception:
    st.error("Hiba a titkos kulcsok betöltésekor. Ellenőrizd a konfigurációt.")
    st.stop()


ai.configure(api_key=api_key)

# Rendszerutasítás a Gemini modell számára
system_instruction = (
...
def display_and_store_message(role, content):
    """Megjeleníti az üzenetet a chaten és eltárolja a session state-ben."""
    with st.chat_message(role):
        st.markdown(content)
    st.session_state.messages.append({"role": role, "content": content})


# --- 2. Streamlit Felület (Frontend) ---
...
if prompt := st.chat_input("Tedd fel a kérdésedet..."):
    # 1. Felhasználói üzenet hozzáadása és megjelenítése
    display_and_store_message("user", prompt)

    # 2. Válasz generálása (Streaming)
    with st.chat_message("assistant"):
        with st.spinner("Válasz generálása..."):
            try:
                # Üzenet küldése a Gemini-nek streaming módban
                response_stream = st.session_state.chat.send_message_stream(prompt)

                # Helykitöltő az üzenet dinamikus frissítéséhez
                message_placeholder = st.empty()
                full_response = ""

                # Végigiterálunk a stream-en, folyamatosan megjelenítve a szöveget
                for chunk in response_stream:
                    full_response += chunk.text
                    # Frissítjük a helykitöltőt a gépelés hatásért (a "▌" kurzort szimulál)
                    message_placeholder.markdown(full_response + "▌")

                # Kicseréljük a végső, teljes válasszal (lezáró kurzor nélkül)
                message_placeholder.markdown(full_response)

                # 3. Válasz hozzáadása a memóriához (csak a végső, teljes válasz)
                st.session_state.messages.append({"role": "assistant", "content": full_response})

            except Exception as e:
                error_message = f"Hiba a válasz generálása során: {e}"
                st.error(error_message)
                # Hibaüzenetet is hozzáadjuk a memóriához
